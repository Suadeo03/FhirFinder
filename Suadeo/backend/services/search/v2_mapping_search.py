...
class V2FHIRSearchService:   
    def __init__(self):
        ...
        try:
            ...
            self.query_tracker = QueryTracker()  # Add query tracker
        except Exception as e:
            print(f"V2FHIRSearchService failed to connect to Chroma: {e}")
            self.collection = None
            self.chroma_config = None
            self.query_tracker = QueryTracker()  # Still init tracker even if Chroma fails

    def semantic_search(self, query: str, limit: int = 10, db: Session = None, filters: Optional[Dict] = None) -> List[Dict]:
        """Semantic search for V2-FHIR mappings with comprehensive tracking"""
        
        # Start query tracking
        query_id = self.query_tracker.start_query_tracking(
            query_text=query,
            search_type='semantic',
            dataset_type='v2fhir',
            user_session='web-session'  # You can pass this from the API
        )
        ...
        try:
            ...
            if not phi_scrubbed_query or not phi_scrubbed_query.strip():
                ...
                # Track failed query
                self.query_tracker.complete_query_tracking([], filters, db)
                return []
            ...
            if not cached_feedback:
                ...
                self.query_tracker.mark_embedding_start()
                query_embedding = self.model.encode([phi_scrubbed_query])[0].tolist()
                self.query_tracker.mark_embedding_end()
                ...
                # Track Chroma search
                self.query_tracker.mark_chroma_start()
                ...
                self.query_tracker.mark_chroma_end()
            ...
            self.query_tracker.complete_query_tracking(results, filters, db)
