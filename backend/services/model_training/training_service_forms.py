...
from models.database.form_model import Form
from config.chroma import get_chroma_instance, is_chroma_available

class FeedbackTrainingForm:
    def __init__(self):
        ...
        try:
            ...
            print("✅ FeedbackTrainingForm connected to Chroma singleton")
    ...
    def record_user_feedback(self, query: str, form_id: str, feedback_type: str, 
                       user_id: str, session_id: str, original_score: float, 
                       db: Session, context_info: Optional[Dict] = None):

    
        profile = db.query(Form).filter(Form.id == form_id).first()
        
        if not profile:
            print(f"Profile {form_id} not found in database")
            return {"status": "error", "message": "Profile not found"}
        
        if not profile.is_active:
            print(f"Profile {form_id} is inactive, skipping embedding update")
            self._store_feedback_record(query, form_id, feedback_type, user_id, 
        ...
        if feedback_type in ['positive', 'negative']:
            self._update_profile_embedding(query, form_id, feedback_type, db)
    ...
    def _update_profile_embedding(self, query: str, form_id: str, feedback_type: str, db: Session):
        try:
            ...
            if not self._is_valid_embedding_result(current_results):
                print(f"Invalid embedding result for {form_id}")
                return
            ...
            if np.any(np.isnan(current_embedding)):
                print(f"NaN values in embedding for profile {form_id}")
                return
            ...
            if embedding_norm > 1e-8:  
                ...
                print(f"Warning: Near-zero embedding norm for profile {form_id_id}, keeping original")
                updated_embedding = current_embedding

            if np.any(np.isnan(updated_embedding)) or np.any(np.isinf(updated_embedding)):
                print(f"Invalid updated embedding for profile {form_id}, keeping original")
                updated_embedding = current_embedding
            ...
            print(f"Successfully updated embedding for profile {form_id} based on {feedback_type} feedback")
