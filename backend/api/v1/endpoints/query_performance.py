    ...
    try:
        ...
        for record in logfile_records:
            data.append({
                ...
                'keywords': record.keywords if isinstance(record.keywords, (dict, list)) else {"raw": record.keywords}
            })
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=f'.{format}') as tmp_file:
            if format.lower() == 'excel':
                ...
                filename = f"performance_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            else:  
